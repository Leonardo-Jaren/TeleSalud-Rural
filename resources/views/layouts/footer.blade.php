@php use Illuminate\Support\Facades\Auth; @endphp
<footer class="bg-dark text-white mt-auto">
    <div class="container py-4">
        @if(Auth::check())
            @php $role = Auth::user()->rol ?? 'paciente'; @endphp
            
            <div class="row">
                <div class="col-md-4 mb-3 mb-md-0">
                    <h5 class="mb-3">
                        <span class="logo me-2">TS</span>
                        {{ config('app.name', 'TeleSalud Rural') }}
                    </h5>
                    <p class="text-white-50 small mb-0">
                        Atención médica remota de calidad para comunidades rurales
                    </p>
                </div>

                <div class="col-md-4 mb-3 mb-md-0">
                    <h6 class="text-uppercase mb-3">Navegación - {{ ucfirst($role) }}</h6>
                    <ul class="list-unstyled mb-0">
                        <li class="mb-2">
                            <a class="text-white-50 text-decoration-none" href="{{ route('home') }}">
                                <i class="bi bi-house-door me-2"></i>Inicio
                            </a>
                        </li>

                        @if($role === 'admin')
                            <li class="mb-2">
                                <a class="text-white-50 text-decoration-none" href="{{ route('admin.dashboard') }}">
                                    <i class="bi bi-speedometer2 me-2"></i>Administración
                                </a>
                            </li>
                            <li class="mb-2">
                                <a class="text-white-50 text-decoration-none" href="{{ route('admin.usuarios') }}">
                                    <i class="bi bi-people me-2"></i>Usuarios
                                </a>
                            </li>
                            <li class="mb-2">
                                <a class="text-white-50 text-decoration-none" href="{{ route('admin.registrar-medico') }}">
                                    <i class="bi bi-person-plus me-2"></i>Registrar Médico
                                </a>
                            </li>
                        @endif

                        @if($role === 'medico')
                            <li class="mb-2">
                                <a class="text-white-50 text-decoration-none" href="{{ route('medico.dashboard') }}">
                                    <i class="bi bi-calendar-check me-2"></i>Mis Horarios
                                </a>
                            </li>
                            <li class="mb-2">
                                <a class="text-white-50 text-decoration-none" href="{{ route('medico.perfil') }}">
                                    <i class="bi bi-person-badge me-2"></i>Mi Perfil
                                </a>
                            </li>
                        @endif

                        @if($role === 'paciente')
                            <li class="mb-2">
                                <a class="text-white-50 text-decoration-none" href="{{ route('paciente.dashboard') }}">
                                    <i class="bi bi-calendar-plus me-2"></i>Reservar Cita
                                </a>
                            </li>
                            <li class="mb-2">
                                <a class="text-white-50 text-decoration-none" href="{{ route('paciente.historial') }}">
                                    <i class="bi bi-clock-history me-2"></i>Historial
                                </a>
                            </li>
                        @endif
                    </ul>
                </div>

                <div class="col-md-4">
                    <h6 class="text-uppercase mb-3">Cuenta</h6>
                    <ul class="list-unstyled mb-0">
                        <li class="mb-2">
                            <span class="text-white-50">
                                <i class="bi bi-person-circle me-2"></i>{{ Auth::user()->name }}
                            </span>
                        </li>
                        <li class="mb-2">
                            <span class="badge bg-primary">{{ ucfirst($role) }}</span>
                        </li>
                        <li class="mb-2">
                            <a class="text-white-50 text-decoration-none" href="{{ route('logout') }}"
                               onclick="event.preventDefault(); document.getElementById('footer-logout-form').submit();">
                                <i class="bi bi-box-arrow-right me-2"></i>Cerrar Sesión
                            </a>
                            <form id="footer-logout-form" action="{{ route('logout') }}" method="POST" class="d-none">
                                @csrf
                            </form>
                        </li>
                    </ul>
                </div>
            </div>

            <hr class="my-3 border-secondary">
            
            <div class="row">
                <div class="col-12 text-center">
                    <p class="text-white-50 small mb-0">
                        &copy; {{ date('Y') }} {{ config('app.name', 'TeleSalud Rural') }}. Todos los derechos reservados.
                    </p>
                </div>
            </div>
        @else
            <div class="row">
                <div class="col-md-6 mb-3 mb-md-0">
                    <h5 class="mb-3">
                        <span class="logo me-2">TS</span>
                        {{ config('app.name', 'TeleSalud Rural') }}
                    </h5>
                    <p class="text-white-50 small">
                        Atención médica remota de calidad para comunidades rurales
                    </p>
                </div>
                <div class="col-md-6 text-md-end">
                    <a href="{{ route('login') }}" class="btn btn-outline-light btn-sm me-2">Iniciar Sesión</a>
                    <a href="{{ route('register') }}" class="btn btn-light btn-sm">Registrarse</a>
                </div>
            </div>
            
            <hr class="my-3 border-secondary">
            
            <div class="row">
                <div class="col-12 text-center">
                    <p class="text-white-50 small mb-0">
                        &copy; {{ date('Y') }} {{ config('app.name', 'TeleSalud Rural') }}. Todos los derechos reservados.
                    </p>
                </div>
            </div>
        @endif
    </div>
</footer>

<style>
footer a:hover {
    color: #fff !important;
    transition: color 0.3s ease;
}
footer .logo {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    width: 32px;
    height: 32px;
    background: linear-gradient(135deg, var(--brand-primary, #10b981) 0%, var(--brand-primary-600, #059669) 100%);
    border-radius: 8px;
    font-weight: 700;
    font-size: 14px;
    color: white;
}
</style>
